/**
 * Admin handlers — user management, stats, feature flags.
 */
...
export const getAdminUser = withAdmin(async ({ req, res }) => {
  ...
  const uid = extractPathParam(req.path, "/admin/users/");
...
export const updateFeatureFlag = withAdmin(async ({ req, res }) => {
  const db = admin.firestore();
  const flagId = extractPathParam(req.path, "/admin/feature-flags/");
